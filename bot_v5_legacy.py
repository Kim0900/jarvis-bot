...
async def ocr_call_card(image_bytes: bytes) -> dict | None:
    prompt = (
        "이 콜카드(운행이력) 이미지에서 아래 JSON만 반환해줘.\n"
        '{"날짜":"YYYY-MM-DD","배차시각":"HH:MM","하차시각":"HH:MM",'
        '"출발지":"OO구 OO동","도착지":"OO구 OO동",'
        '"요금":숫자,"카드사":"카드사명","콜유형":"카카오T 또는 우버 또는 배회","결제방식":"자동 또는 직접"}\n'
        "⚠️ 콜유형 판별(대표님 지적, 2026-08-26 긴급수정 — 이전엔 '우버' 선택지가 "
        "없어서 우버콜이 전부 배회로 잘못 분류되던 버그):\n"
        "  - 카카오T 앱 화면(로고/UI가 카카오T, '일별 운행 이력' 제목) → 카카오T\n"
        "  - 우버 앱 화면(로고/UI가 Uber, '운행 세부사항'·'순수익'·'수입 포인트' "
        "같은 표현, 지도가 화면 상단에 크게 표시) → 우버\n"
        "  - 카카오T도 우버도 아닌 앱 화면 밖 수기입력/기타 → 배회\n"
        "⚠️ 날짜: 이미지 상단의 운행 날짜(예: 2026/03/07 → 2026-03-07). 없으면 null.\n"
        "  단, 화면에 여러 날짜가 있으면 가장 최근 운행의 날짜만 추출.\n"
        "⚠️ 요금 추출:\n"
        "  - \'미터 요금\', \'총 요금\', \'결제 금액\', \'이용 요금\', \'순수익\' 레이블 옆 숫자\n"
...
#
# 2026-09-07 회귀수정(영실 지시서 CONFIRMED): 9/6 커밋(5ce8626)에서
# 로컬 캐시 파일(9/3 시점 구버전)을 기반으로 작업하다 9/5 Tesseract
