...
async def process_call_card(image_bytes, update):
    ...
    now=datetime.now(KST); call_date=now.date()
    배차시각=data.get("배차시각","")
    if 배차시각:
        try:
            h=int(배차시각.split(":")[0])
...
async def handle_realtime(text, update):
    hm=re.search(r'(\d{1,2})시',text); hour=int(hm.group(1)) if hm else datetime.now(KST).hour
    ...
    prompt=(f"대구 전기차 택시 전략 AI 자비스야.\n"
            ...
            f"지금 즉시 할 행동 3줄. 대표님이라고 불러줘. 이모지 활용. 반드시 일반 텍스트로만 답해줘. ##, **, --, |, --- 같은 마크다운 기호 절대 사용 금지. 표 형식 사용 금지. 모르는 정보나 DB에 없는 데이터는 절대 지어내지 말 것.\n마크다운 기호(##,**,---,|) 절대 사용 금지. 전략DB에 없는 정보는 절대 지어내지 말 것.")
    try:
        resp=claude_client.messages.create(model="claude-sonnet-4-6",max_tokens=250,
            messages=[{"role":"user","content":prompt}])
        advice=resp.content[0].text.strip()
    except: advice=strategy or "현재 시간대 전략 데이터가 없습니다. 잠시 후 다시 확인해주세요."
...
async def handle_report(update):
    ...
    try:
        prompt=(f"대구 전기차 택시 전략 AI 자비스야.\n"
                ...
                f"✅ 잘한 점:\n📌 개선점:\n🎯 내일 핵심전략:\n각각 한 줄. 대표님이라고 불러줘. 반드시 일반 텍스트로만 답해줘. ##, **, --, |, --- 같은 마크다운 기호 절대 사용 금지. 표 형식 사용 금지. 모르는 정보나 DB에 없는 데이터는 절대 지어내지 말 것.")
...
async def handle_text(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["전략","실시간","전략브리핑","브리핑"]:
        h=datetime.now(KST).hour; s=get_strategy(h)
    ...
    prompt=(f"대구 전기차 택시 전략 AI 자비스야.\n"
            ...
            f"대표님 메시지: {text}\n간결하고 실용적으로 답해줘. 대표님이라고 불러줘. 반드시 일반 텍스트로만 답해줘. ##, **, --, |, --- 같은 마크다운 기호 절대 사용 금지. 표 형식 사용 금지. 모르는 정보나 DB에 없는 데이터는 절대 지어내지 말 것.")
    try:
