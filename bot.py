...
답변: 위 5가지 중 하나만."""
    ...
    for t in ["call_card","charging","sekuti","demand"]:
        if t in answer:
            return t
...
def get_strategy(hour: int) -> dict | None:
    try:
        r = supabase.table("strategy_lookup").select("*").execute()
        rows = r.data or []
    except Exception as e:
        logger.error(f"strategy_lookup 오류: {e}")
        return None
    weekday = ["월","화","수","목","금","토","일"][now_kst().weekday()]
    def h_match(시간대):
        if not 시간대 or 시간대 == "전체": return True
        try:
            t = 시간대.replace("시","").strip().split("~")
            s, e = int(t[0]), int(t[1])
            return (s <= hour < e) if s <= e else (hour >= s or hour < e)
        except: return False
    def d_match(요일):
        if not 요일 or 요일 == "전체": return True
        return weekday in 요일
    matched = [r for r in rows if h_match(r.get("시간대","")) and d_match(r.get("요일",""))]
    if not matched:
        matched = [r for r in rows if h_match(r.get("시간대",""))]
    if not matched: return None
    matched.sort(key=lambda x: 0 if x.get("우선순위") in ("높음","긴급") else 1)
    return matched[0]
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "대조":
        ...
        pending = r.data or []
        if not pending:
            await update.message.reply_text("임시기록 없음.")
            return
        lines = ["📋 임시기록 목록"]
        for p in pending:
...
def _run_health():
    HTTPServer(("0.0.0.0", int(os.environ.get("PORT",10000))), _Health).serve_forever()
