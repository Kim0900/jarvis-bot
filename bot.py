...
def get_strategy(hour: int) -> dict | None:
    """strategy_lookup에서 현재 시각 기준 전략 조회"""
    r = supabase.table("strategy_lookup").select("*")\
        .lte("start_hour", hour).gte("end_hour", hour).execute()
    data = r.data
...
async def generate_strategy(context_text: str = "") -> str:
    ...
    if strategy_row:
        base = strategy_row.get("strategy_text", "")
        # 현재 시각 명시적으로 포함
        return (
...
async def post_init(application):
    """이벤트 루프 시작 후 스케줄러 초기화 (BUG-11 수정)"""
