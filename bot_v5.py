...
async def calc_kpi_metrics(날짜: str, 매출: int, work_hours) -> dict:
    ...
    window_calls = await sb_select("raw_calls", {
        "and": f"(날짜.gte.{window_start},날짜.lte.{날짜})"
    }) or []

    def _axis_b_date(축a_날짜, 배차시각):
        if not 축a_날짜 or not 배차시각:
            return 축a_날짜
        try:
            h = int(str(배차시각).split(":")[0])
        except Exception:
            return 축a_날짜
        if h < 6:  # 0~5시 배차 = 전날 심야운행의 익일 새벽 연장
            try:
                return (_dc.fromisoformat(축a_날짜) + _td(days=1)).isoformat()
            except Exception:
                return 축a_날짜
        return 축a_날짜

    # 축B 일별 카카오T 완료건수 (최근 7일, 콜 없는 날은 0으로 유지)
    ...
    for c in window_calls:
        ...
        if b_date in axis_b_counts:
            axis_b_counts[b_date] += 1
    kpi_7day_avg = round(sum(axis_b_counts.values()) / 7, 2)

    # 평균단가 — 오늘(축A) 카카오T 콜 기준, 평일(월~금)만 산출
    weekday = target.weekday()  # 0=월 ... 5=토 6=일
    today_kakao = [c for c in window_calls
                   if c.get("날짜") == 날짜 and (c.get("콜유형") or "") == "카카오T"]
    if weekday <= 4 and today_kakao:
        kpi_avg_fare = int(sum(c.get("요금", 0) or 0 for c in today_kakao) / len(today_kakao))
    else:
        kpi_avg_fare = None
    ...
    if week_kakao:
        longdist = sum(1 for c in week_kakao if (c.get("요금", 0) or 0) >= 15000)
        kpi_longdist_rate = round(longdist / len(week_kakao) * 100, 1)
    else:
...
async def handle_briefing(update, date_str: str = None):
    ...
    # 캐스퍼 명령서 #006 §3 / #008 §3 반영 — work_start/end/hours + KPI 4종 통합 저장
    # 주의: 19시~02시처럼 자정을 넘는 심야 운행이 대부분이므로 단순 문자열 정렬은
    # "00:20"이 "23:50"보다 앞선 것으로 오판함 → 새벽(0~5시)은 가상으로 +24h 처리해서 정렬
    try:
        ...
        # 명령서 #008 §3 — KPI 4종 (봇 자체 계산, 축B 필수)
