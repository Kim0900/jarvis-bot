...
- 순수익 목표: 100,000원/일 (카드수수료 3.3% 역산)
- 일일 보험료 자동 기록: 7,945원 (290만원/365일)
- 지출 관리: 충전/타이어/오일/세차/기타
- 휴무일 자동 처리
- Render Web Service 호환 (health check 포함)
"""
...
AVG_FARE        = 9504
AVG_NET_FARE    = round(AVG_FARE * (1 - CARD_FEE_RATE))
...
def sb_insert(t, d):
    try:
        ...
        logger.error(f"insert error: {e}"); return False
...
def sb_select(t, p=""):
    try:
        ...
        logger.error(f"select error: {e}"); return []
...
def sb_upsert(t, d, c):
    try:
        ...
        logger.error(f"upsert error: {e}"); return False

def sb_delete_last(t, target_date):
    try:
        rows = sb_select(t, f"날짜=eq.{target_date}&자동여부=eq.false&order=id.desc&limit=1&select=id")
...
def auth_check(update): return update.effective_chat.id == ALLOWED_CHAT_ID

def build_report(td):
    dow = get_dow(td)
    s   = today_summary(td)
    exp = today_expenses(td)
    순수익 = s["순매출"] - exp["total"]
    달성률 = round(순수익/NET_GOAL*100,1) if NET_GOAL else 0
...
def parse_expense(text):
    ...
    if text.startswith("지출"):
        parts = text.split()
        memo = parts[1] if len(parts)>2 else "기타"
...
async def process_call_card(image_bytes, update):
    ...
    msg = f"📥 *저장완료* — {call_date.strftime('%m/%d')} ({요일})\n\n{'🚖' if 유형=='카카오T' else '🚶'} {배차시각 or '시각미확인'} | {data.get('출발지','미확인')} → {data.get('도착지','미확인')}\n💰 {fmt(data.get('요금',0))} | {유형}{g_tag}\n━━━━━━━━━━━━━━━━━━━━\n📊 *오늘 누계*\n  건수: {s['건수']}건 | 매출: {fmt(s['매출'])}\n  순수익: {fmt(순수익)}\n  목표({fmt(NET_GOAL)}) 달성률: *{달성률}%*\n  잔여: {fmt(잔여)} (약 {잔여콜}콜)"
    if 순수익>=NET_GOAL: msg += "\n  🎉 *오늘 목표 달성!*"
    if strategy: msg += f"\n━━━━━━━━━━━━━━━━━━━━\n🎯 *지금 행동*\n{strategy}"
...
async def handle_expense(text, update):
    ...
    순수익 = s["순매출"]-exp["total"]
    msg = f"💸 *지출 기록완료*\n{cat}: {fmt(amount)}\n━━━━━━━━━━━━━━━━━━━━\n오늘 지출합계: {fmt(exp['total'])}\n현재 순수익: {fmt(순수익)}\n목표 달성률: {round(순수익/NET_GOAL*100,1)}%"
    await update.message.reply_text(msg, parse_mode="Markdown")
...
async def handle_realtime(text, update):
    ...
    prompt=f"대구 전기차 택시 전략 AI 자비스야.\n현재: {hour}시 | 위치: {loc or '미확인'} | {f'공차 {gm.group(1)}분 경과' if gm else ''}\n오늘({dow}): {s['건수']}건 순수익 {fmt(순수익)} / 목표 {fmt(NET_GOAL)}\n전략DB: {strategy or '일반운행'}\n지금 즉시 할 행동 3줄. 대표님이라고 불러줘. 이모지 활용."
    try:
        resp=claude_client.messages.create(model="claude-sonnet-4-6",max_tokens=250,messages=[{"role":"user","content":prompt}])
...
async def handle_report(update):
    ...
    try:
        prompt=f"대구 전기차 택시 전략 AI 자비스야.\n오늘({dow}): {s['건수']}건 / 순수익 {fmt(순수익)} / 달성률 {round(순수익/NET_GOAL*100,1)}%\n내일은 {tomorrow_dow}요일.\n✅ 잘한 점:\n📌 개선점:\n🎯 내일 핵심전략:\n각각 한 줄. 대표님이라고 불러줘."
        resp=claude_client.messages.create(model="claude-sonnet-4-6",max_tokens=200,messages=[{"role":"user","content":prompt}])
        msg+=f"\n━━━━━━━━━━━━━━━━━━━━\n{resp.content[0].text.strip()}"
    except: pass
    await update.message.reply_text(msg,parse_mode="Markdown")
    sb_upsert("daily_summary",{"날짜":str(today),"요일":dow,"총건수":s["건수"],"총매출":s["매출"],"배회건수":s["배회"],"정상여부":"정상","휴무여부":False},"날짜")
...
async def handle_weekly(update):
    ...
    msg=f"📈 *이번 주 성과*\n━━━━━━━━━━━━━━━━━━━━\n기간: {ws.strftime('%m/%d')} ~ {today.strftime('%m/%d')}\n운행일: {days}일 | 총건수: {cnt}건\n총매출: {fmt(tr)} | 지출: △{fmt(te)}\n━━━━━━━━━━━━━━━━━━━━\n✅ 주간 순수익: *{fmt(np)}*\n일평균 순수익: {fmt(np//days if days else 0)}"
    await update.message.reply_text(msg,parse_mode="Markdown")
...
# ── 스케줄러 ──────────────────────────────────────
async def auto_insurance_job():
    today=date.today(); insert_insurance(today)
    logger.info(f"보험료 자동기록: {today} {fmt(INSURANCE_DAILY)}")
...
async def handle_text(update:Update, context:ContextTypes.DEFAULT_TYPE):
    ...
    today=date.today(); dow=get_dow(today); s=today_summary(today); exp=today_expenses(today)
    순수익=s["순매출"]-exp["total"]
    prompt=f"대구 전기차 택시 전략 AI 자비스야.\n오늘 {dow}요일 | {s['건수']}건 | 순수익 {fmt(순수익)} / 목표 {fmt(NET_GOAL)}\n대표님 메시지: {text}\n간결하고 실용적으로 답해줘. 대표님이라고 불러줘."
    try:
        resp=claude_client.messages.create(model="claude-sonnet-4-6",max_tokens=300,messages=[{"role":"user","content":prompt}])
...
def main():
    ...
    scheduler=AsyncIOScheduler(); scheduler.add_job(auto_insurance_job,"cron",hour=0,minute=1); scheduler.start()
